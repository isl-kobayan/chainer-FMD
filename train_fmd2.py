...
parser.add_argument('--train', '-f', default='test_list.txt',
...
# Prepare dataset
train_list_base = fmdio.load_image_list(args.train, args.root)
train_list = []
val_list = []
material_size=10
per_category=len(train_list_base)/material_size
for m in six.moves.range(material_size):
    tuples=train_list_base[m*per_category:(m+1)*per_category]
    random.shuffle(tuples)
    train_list += tuples[0:per_category/2]
    val_list += tuples[per_category/2:per_category]
...
outdir = './' + args.arch + '_bs' + str(args.batchsize) + '_' + nowt.strftime("%Y%m%d-%H%M")
os.mkdir(outdir)
