...
def train(args):
    ...
    # switch
    switch = {'conv3': np.random.rand(384) <= 0.95,
                'conv4': np.random.rand(384) <= 0.9}
    ...
    def feed_data():
        ...
        for epoch in six.moves.range(1, 1 + args.epoch):
            ...
            for idx in perm:
                ...
                batch_pool[i] = pool.apply_async(fmdio.read_image, (path, model.insize, mean, False, True))
                y_batch[i] = label
                i += 1
                ...
                if count % args.val_interval == 0:
                    ...
                    for path, label in val_list:
                        val_batch_pool[j] = pool.apply_async(
                            fmdio.read_image, (path, model.insize, mean, True, False))
                        val_y_batch[j] = label
                        j += 1
        ...
        while True:
            ...
            if train:
                ...
                if train_count % iterates_per_epoch == 0:
                    ...
                    train_cur_loss = 0
                    train_cur_accuracy = 0
                    Ret[0].train_loss = mean_loss
                    Ret[0].train_error = mean_error
            else:
    ...
    def train_loop():
        ...
        while True:
            ...
            if model.train:
                optimizer.update(model, x, t, switch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', '-a', choices=models.archs.keys(), default='alex-with-switch',
